...
class MySQl_info {
    ...
    public function createSQL($tablename){
        ...
        $sql_zbtb = <<<sql
                ...
                name char(5) NOT NULL UNIQUE KEY COMMENT '节目名',
                hdurl varchar(100) COMMENT '超清',
                mdurl varchar(100) COMMENT '高清',
                udurl varchar(100) COMMENT '标清',
                tdurl varchar(100) COMMENT '流畅',
                time  int(10) NOT NULL DEFAULT $time COMMENT '最后修改时间'
...
sql;
    ...
    public  function wSQL($tablename,$arr=[]){
        ...
        $writ_key  =implode(',',$keys).',time';
        $writ_value ="'".implode("','",$values)."','".time()."'";
        $keys = null;
        foreach ($arr as $key => $value){
            if($key!='type' && $key!='id'){
                $keys[]=$key."='".$value."'";
