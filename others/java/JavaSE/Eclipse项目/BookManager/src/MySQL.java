...
public class MySQL{
		...
		if(jdbc ==null || stock==null || Name==null || Pass==null) {
			...
			switch(jdbc) {
				case "MySQL": 
					...
					this.dbUrl = "jdbc:mysql://"+url+":"+port+"/"+stock+"?useSSL=false&serverTimezone=GMT&useUnicode=rue;characterEncoding="+encode;
				break;
				case "SQLServer": 
	...
	public void addData(String table,String[] fields,String[] values) {
		...
		field = toStr(fields,"",",");
		value = toStr(values,"'","',");
		System.out.println("INSERT INTO "+table+"("+field+") VALUES("+value+");");
		//执行语句
	...
	//验证数据
	public boolean[] dataTest(String endsql,String[] fileds,String[] datas) {
		ResultSet rs=null;
		//boolean  f = false;
		PreparedStatement psmt=null;
		int len = datas.length;
		boolean[] ff= new  boolean[len];
		String sql = "SELECT * FROM "+endsql;
		try {
			psmt = conn.prepareStatement(sql);
			rs = psmt.executeQuery();
			while(rs.next()) {
				for(int i=0;i < len;i++) { 
					ff[i]=rs.getString(fileds[i]).equals(datas[i]);
				}
			}
			// 完成后关闭
			if(psmt!=null)psmt.close();
